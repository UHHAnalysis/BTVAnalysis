...
BTagEffHistsTopTagJim::BTagEffHistsTopTagJim(const char* name, float csv) : BaseHists(name)
{
...
BTagEffHistsTopTagJim::~BTagEffHistsTopTagJim()
{
    // default destructor, does nothing
}


void BTagEffHistsTopTagJim::Init()
{
...
void BTagEffHistsTopTagJim::Fill()
{
    ...
    for (unsigned int i =0; i<jets->size(); ++i) {
      ...
      double deltar=bcc->jets->at(i).deltaR(bcc->muons->at(0));

      if(!((deltar<(PI/2))&&(bcc->jets->at(i).pt()>30.)&&(fabs(bcc->jets->at(i).eta())<2.4))) continue;
...
void BTagEffHistsTopTagJim::Finish()
{
